...
const getCoupons = () => {
  try {
    ...
    return s
      ? JSON.parse(s).filter((c: any) => c.isActive)           // อ่านจาก localStorage + กรองเฉพาะ active
      : availableCoupons.map(c => ({ ...c, isActive: true })); // fallback ถ้ายังไม่มี Admin จัดการ
  } catch { return []; }
};

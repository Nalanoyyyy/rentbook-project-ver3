...
const PER_PAGE = 12;
const SUB_CATEGORIES = ['ทั้งหมด', 'แอคชั่น', 'แฟนตาซี', 'มังงะ', 'ผจญภัย', 'ดราม่า'];
...
const Cartoon: React.FC = () => {
 ...
 const load = useCallback(async () => {
  ...
  const filtered = category === 'ทั้งหมด'
    ? cartoonBooks
    : cartoonBooks.filter(b => (b.category || '').split(',').map((s: string) => s.trim()).includes(category));
  ...
  return (
    <main className="max-w-[1200px] mx-auto px-4 md:px-10 py-6">
      <div className="mb-8 flex justify-between items-end">
        <div><h1 className="text-3xl font-bold mb-2">หนังสือการ์ตูน</h1><p className="text-gray-500">รวมมังงะและคอมมิคสุดมันส์</p></div>
        ...
        <aside className="lg:col-span-1 space-y-8">
          <div className="bg-white dark:bg-white/5 p-6 rounded-3xl border border-[#e7f3ec] dark:border-[#1a3324]">
            <h3 className="font-bold text-lg mb-4 flex items-center gap-2">
              <span className="w-1.5 h-6 bg-primary rounded-full" />ประเภทการ์ตูน
            </h3>

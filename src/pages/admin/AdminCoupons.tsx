...
import { apiGetAllCoupons, apiCreateCoupon, apiUpdateCoupon, apiDeleteCoupon } from '../../services/api';
import AdminNav from './AdminNav';

type Coupon = { id: number; code: string; title: string; type: string; discountValue: number; displayDiscount: string; minSpend: number; expiry: string; isActive: boolean; usageCount: number; totalDiscount: number };
type FormState = Omit<Coupon, 'id' | 'isActive' | 'usageCount' | 'totalDiscount'>;
...
const Field: React.FC<{ label: string; required?: boolean; error?: string; hint?: string; children: React.ReactNode }> =
  ({ label, required, error, hint, children }) => (
    <div>
      <label className="block text-xs font-bold uppercase tracking-wider text-gray-500 mb-1.5">{label}{required && <span className="text-red-400 ml-0.5">*</span>}</label>
      {children}
...
const ModalShell: React.FC<{ onClose: () => void; maxW?: string; children: React.ReactNode }> = ({ onClose, maxW = 'max-w-lg', children }) => (
...
const CouponFormModal: React.FC<{ coupon: Coupon | null; onClose: () => void; onSave: (d: FormState, id?: number) => void }> =
  ({ coupon, onClose, onSave }) => {
    const isEdit = coupon !== null;
    const [form, setForm] = useState<FormState>(isEdit ? { code: coupon.code, title: coupon.title, type: coupon.type, discountValue: coupon.discountValue, displayDiscount: coupon.displayDiscount, minSpend: coupon.minSpend, expiry: coupon.expiry } : EMPTY_FORM);
    const [errors, setErrors] = useState<Partial<Record<keyof FormState, string>>>({});
    const set = (k: keyof FormState, v: any) => setForm(p => {
      const next = { ...p, [k]: v };
      if ((k === 'type' || k === 'discountValue') && next.discountValue) next.displayDiscount = toDisplay(next.type, next.discountValue);
      return next;
    });
    const handleSubmit = () => {
    ...
    return (
      <ModalShell onClose={onClose}>
        <div className="flex items-center justify-between p-6 border-b border-gray-100 dark:border-white/10 bg-gray-50 dark:bg-white/5">
          <h3 className="text-xl font-bold flex items-center gap-2"><span className="material-symbols-outlined text-primary">{isEdit ? 'edit' : 'add_circle'}</span>{isEdit ? 'แก้ไขคูปอง' : 'เพิ่มคูปองใหม่'}</h3>
          <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-full bg-gray-200 dark:bg-white/10 hover:bg-red-500 hover:text-white transition-colors"><span className="material-symbols-outlined text-[20px]">close</span></button>
        </div>
        <div className="p-6 space-y-4 max-h-[65vh] overflow-y-auto">
          <div className="grid grid-cols-2 gap-4">
            <Field label="รหัสคูปอง" required error={errors.code} hint="ตัวพิมพ์ใหญ่ ไม่มีช่องว่าง">
              <input type="text" className={inputCls} placeholder="SUMMER20" value={form.code} onChange={e => set('code', e.target.value.toUpperCase().replace(/\s/g, ''))} />
            </Field>
          ...
          <Field label="ชื่อคูปอง" required error={errors.title}>
            <input type="text" className={inputCls} placeholder="ส่วนลดสมาชิกใหม่" value={form.title} onChange={e => set('title', e.target.value)} />
          </Field>
          <div className="grid grid-cols-2 gap-4">
            <Field label={`ส่วนลด (${form.type === 'PERCENT' ? '%' : '฿'})`} required error={errors.discountValue}>
              <input type="number" min={0} max={form.type === 'PERCENT' ? 100 : undefined} className={inputCls} placeholder="0" value={form.discountValue || ''} onChange={e => set('discountValue', Number(e.target.value))} />
            </Field>
            <Field label="ยอดขั้นต่ำ (฿)" hint="0 = ไม่กำหนด">
              <input type="number" min={0} className={inputCls} placeholder="0" value={form.minSpend || ''} onChange={e => set('minSpend', Number(e.target.value))} />
            </Field>
          </div>
          <Field label="วันหมดอายุ" required error={errors.expiry} hint='เช่น "31 ธ.ค. 2026"'>
            <input type="text" className={inputCls} placeholder="31 ธ.ค. 2026" value={form.expiry} onChange={e => set('expiry', e.target.value)} />
          </Field>
        </div>
        <div className="p-6 border-t border-gray-100 dark:border-white/10 bg-gray-50 dark:bg-white/5 flex justify-end gap-3">
          ...
          <button onClick={handleSubmit} className="px-6 py-2 rounded-xl font-bold bg-primary text-white hover:bg-primary/90 transition-colors shadow-lg shadow-primary/20 text-sm">{isEdit ? 'บันทึกการแก้ไข' : 'เพิ่มคูปอง'}</button>
        </div>
      </ModalShell>
    );
  };
...
const AdminCoupons: React.FC = () => {
  ...
  const load = useCallback(async () => {
    try { setCoupons(await apiGetAllCoupons()); }
    catch { navigate('/'); }
  }, [navigate]);
  ...
  const handleSave = useCallback(async (data: FormState, id?: number) => {
    try {
      if (id) await apiUpdateCoupon(id, { ...data, isActive: true });
      else    await apiCreateCoupon(data);
      await load();
      showToast(id ? 'แก้ไขคูปองเรียบร้อยแล้ว' : 'เพิ่มคูปองใหม่เรียบร้อยแล้ว');
      setFormTarget(null);
  ...
  const handleDelete = useCallback(async () => {
    ...
    try {
      await apiDeleteCoupon(deleteTarget.id);
      await load();
  ...
  const toggleActive = useCallback(async (c: Coupon) => {
    try {
      await apiUpdateCoupon(c.id, { ...c, isActive: !c.isActive });
      await load();
    } catch (err: any) { showToast(err.message || 'เกิดข้อผิดพลาด'); }
  }, [load]);

  const typeCls = (t: string) => t === 'PERCENT' ? 'bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300' : 'bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300';
  const STATS = [
    ...
    <main className="max-w-6xl mx-auto px-4 py-12 min-h-[80vh]">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        {STATS.map(s => (
          <div key={s.label} className="bg-white dark:bg-[#1a3324] p-4 rounded-2xl border border-gray-100 dark:border-white/5 flex items-center gap-3">
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${s.bg}`}><span className={`material-symbols-outlined text-xl ${s.color}`}>{s.icon}</span></div>
      ...
      <div className="bg-white dark:bg-[#1a3324] rounded-[2rem] shadow-xl overflow-hidden border border-gray-100 dark:border-white/5">
        <div className="p-5 border-b border-gray-100 dark:border-white/5 flex justify-between items-center">
          <h2 className="font-bold flex items-center gap-2"><span className="material-symbols-outlined text-primary">confirmation_number</span>คูปองทั้งหมด</h2>
          <button onClick={() => setFormTarget('new')} className="flex items-center gap-1.5 px-4 py-2 rounded-xl font-bold bg-primary text-white hover:bg-primary/90 transition-colors shadow-lg shadow-primary/20 text-sm">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
              {coupons.length === 0 ? (
                <tr><td colSpan={7} className="p-12 text-center text-gray-400"><span className="material-symbols-outlined text-4xl mb-2 block">confirmation_number</span>ยังไม่มีคูปองในระบบ</td></tr>
              ) : coupons.map(c => (
                <tr key={c.id} className="border-b border-gray-100 dark:border-white/5 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                  ...
                  <td className="p-4 text-center">
                    <button onClick={() => toggleActive(c)} className={`text-xs font-bold px-3 py-1 rounded-full transition-colors ${c.isActive ? 'bg-green-100 text-green-700 hover:bg-green-200' : 'bg-gray-100 dark:bg-white/10 text-gray-400 hover:bg-gray-200 dark:hover:bg-white/20'}`}>
      ...
      {deleteTarget && (
        <ModalShell onClose={() => setDeleteTarget(null)} maxW="max-w-md">
          <div className="p-8 text-center">
            <div className="w-16 h-16 rounded-full bg-red-100 dark:bg-red-900/30 flex items-center justify-center mx-auto mb-4"><span className="material-symbols-outlined text-red-500 text-3xl">delete_forever</span></div>
            <h3 className="text-xl font-black mb-2">ยืนยันการลบ?</h3>
            <p className="text-gray-500 dark:text-gray-400 mb-6">จะลบคูปอง <span className="font-black text-gray-800 dark:text-white">{deleteTarget.code}</span> ออกจากระบบ<br />การกระทำนี้ไม่สามารถย้อนกลับได้</p>
            <div className="flex gap-3 justify-center">
              <button onClick={() => setDeleteTarget(null)} className="px-6 py-2 rounded-xl font-bold border border-gray-200 dark:border-white/10 hover:bg-gray-100 dark:hover:bg-white/5 transition-colors">ยกเลิก</button>
              <button onClick={handleDelete} className="px-6 py-2 rounded-xl font-bold bg-red-500 text-white hover:bg-red-600 transition-colors shadow-lg shadow-red-500/20">ลบคูปอง</button>
            </div>
          </div>
        </ModalShell>
      )}
